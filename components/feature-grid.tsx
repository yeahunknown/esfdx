  ...
  return (
    ...
    >
      {features.map((feature, index) => (
        <motion.div
          key={index}
          className="flex flex-col items-center space-y-2 rounded-lg border border-neutral-800 bg-[#111111] p-4 transition-all hover:border-[#ccbe43]/50 hover:shadow-md hover:shadow-[#ccbe43]/10"
          variants={item}
          whileHover={{ y: -5, transition: { duration: 0.2 } }}
        >
          <div className="rounded-full bg-[#1d1d1d] border border-[#ccbe43]/20 p-2">{feature.icon}</div>
...
const features = [
  {
    ...
    icon: <Rocket className="h-6 w-6 text-[#ccbe43]" />,
  },
  {
